...
test("legacy vendor-named canonical input remains replayable", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "ajrm-legacy-input-zip-"));
  const zipPath = path.join(root, "parent.zip");
  const targetPath = path.join(root, "replay.jsonl");
  const content = `${JSON.stringify(canonicalInputRecord({
    elapsedMs: 0,
    delta: {
      updates: [{
        $source: "legacy-gateway.2",
        values: [{ path: "navigation.speedOverGround", value: 1 }],
      }],
    },
  }))}\n`;
  const zip = new AdmZip();
  zip.addFile(LEGACY_INPUT_RELATIVE_PATH, Buffer.from(content));
  zip.writeZip(zipPath);
  const result = await extractCanonicalInputFromZip(zipPath, targetPath);
