...
Version `0.10.17` updates `adm-zip` outside the affected range for
GHSA-vwc7-r8mq-g2x9 without changing Capture's ZIP-handling logic. Version
`0.10.16` uses only the canonical sensor-input voyage model, removes the retired
vendor-named input transition, and aligns review/BITE contracts with the
current suite. Version `0.10.15` widens Voyage Review's responsive chart-cycle banner so long
...
```bash
cd ~/.signalk
npm install git+https://github.com/ajrm-marine-suite/signalk-ajrm-marine-capture.git#v0.10.17 --omit=dev --no-package-lock
