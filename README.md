...
Version `0.10.11` serves the Webapps icon from Signal K's public webapp URL and
adds a regression test for the metadata-to-file contract. It retains replay source-time alignment and
...
```bash
cd ~/.signalk
npm install git+https://github.com/ajrm-marine-suite/signalk-ajrm-marine-capture.git#v0.10.11 --omit=dev --no-package-lock
